...
Navigate to the repo settings on Github by clicking the `Settings` tab, and scroll down to the `Github Pages` section, set the `Source` to `main`, and **click the save button**.

![Github Pages Setup](Images/github-pages-setup.gif)

* By setting up the `Source` that Github Pages will use to `main`, we are telling it to build out everything inside of our repository into a website.
